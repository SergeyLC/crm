...
### 🔄 Процесс развертывания (ОПТИМИЗИРОВАННЫЙ)

**Что происходит при deployment (новая оптимизированная версия):**
...
6. **Копирование кода на сервер:** *(GitHub Actions → Server)*
   ```bash
...
7. **Создание environment файлов:** *(GitHub Actions)*
   - Production: `.env.docker` из GitHub Secrets
   - Staging: `.env.backend.stage` + `.env.frontend.stage`

8. **Скачивание готовых образов:** *(GitHub Actions → Server)*
   ```bash
   docker pull ghcr.io/repo/frontend:v1.0.0
   docker pull ghcr.io/repo/backend:v1.0.0
   ```

9. **Миграции базы данных:** *(GitHub Actions → Server)*
   ```bash
   cd db && pnpm run migrate:deploy
   ```

10. **Запуск контейнеров:** *(GitHub Actions → Server)*
    ```bash
    docker-compose up -d  # Использует pre-built образы
    ```
...
- **⚡ Быстрое развертывание:** Pre-built Docker образы сокращают время деплоя на 60-70%
...
- **🔄 Автоматические миграции:** База данных обновляется автоматически при деплое
- **🛡️ Изоляция сред:** Production и staging полностью изолированы
- **📊 Мониторинг:** Все логи и метрики доступны для отладки
...
#### 🏆 **Опция 1: Сборка образов на GitHub Actions (РЕАЛИЗОВАНА)**
- ✅ Собрать Docker образы на CI/CD серверах с кэшированием зависимостей
- ✅ Отправить готовые образы в GitHub Container Registry
- ✅ На сервере только скачать и запустить контейнеры
- **Результат:** Ускорение в **2-3 раза**, время деплоя ~2-3 мин вместо 6-10 мин

**Как работает новая оптимизация:**

1. **GitHub Actions (CI/CD серверы):**
...
2. **Production сервер:**
   ...
   - Запуск контейнеров без повторной сборки
   - Только миграции базы данных

**Время выполнения:**
- **До оптимизации:** ~6-10 мин (сборка на сервере)
- **После оптимизации:** ~2-3 мин (только скачивание и запуск)
- **Ускорение:** ~60-70%
