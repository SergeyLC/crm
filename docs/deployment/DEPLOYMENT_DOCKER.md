...
##### **Опция 1: Сборка образов на GitHub Actions**
- Собрать Docker образы на CI/CD серверах
- Отправить готовые образы в registry (Docker Hub, GitHub Container Registry)
- На сервере только скачать и запустить контейнеры
- **Результат:** Ускорение в 2-3 раза, но требует registry
